...
namespace OrderAssistantUI
{
	...
	public class ImportViewModel : PropertyChangedBase, IImport
	{
		...
		public async void ButtonStartImport()
		{
			...
			{
				Logger.Error("Необработанная ошибка при импорте ({0})", e.Message);
			}
			finally
			{
		...
		private  async Task OrderStocksAndTrafficAsync(string[] fileNames, CancellationToken cancelToken)
		{
			await Task.Run(() =>
			{
				...
				foreach (var fileName in fileNames)
				{
					...
					{
						Logger.Error("Ошибка чтения файла ({0}), {1}", fileName, e.Message);
						ProgressBarValue++;
						continue;
					}
					...
					try
					{
						...
						{
							throw new Exception($"Ячейка содержит значение {dataArr[Config.Inst.Imports.OrderStocksAndTraffic.RowSign, Config.Inst.Imports.OrderStocksAndTraffic.ColSign]}");
						}
					}
					catch (Exception e)
					{
						Logger.Error("Сигнатура файла ({0}) не совпадает с сигнатурой ({1}) {2}", fileName, Config.Inst.Imports.OrderStocksAndTraffic.Sign, e.Message);
						ProgressBarValue++;
						continue;
					}
